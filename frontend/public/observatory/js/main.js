...
class Observatory {
  ...
  _autoDetectLive() {
    ...
    const candidates = [
      { base: window.location.origin, wsPath: '/ws/pose' },   // Same-origin via Vite proxy
      { base: `http://${host}:4000`, wsPath: '/ws/pose' },    // CareWatch backend direct
      { base: window.location.origin, wsPath: '/ws/sensing' },
      { base: `http://${host}:8765`, wsPath: '/ws/sensing' },
      { base: `http://${host}:3000`, wsPath: '/ws/sensing' },
    ];
    // Deduplicate by base URL
    const seen = new Set();
    const unique = candidates.filter(c => seen.has(c.base) ? false : (seen.add(c.base), true));
  ...
  _connectWS(url) {
    ...
    try {
      ...
      this._ws.onclose = () => {
        console.log('[Observatory] WebSocket closed, reconnecting in 3s...');
        this._ws = null;
        // Keep dataSource='ws' and _liveData so last frame persists
        setTimeout(() => this._connectWS(url), 3000);
      };
      this._ws.onerror = () => {};
    } catch {}
  }
