...
export class PoseSystem {
  ...
  // ======================================================================
  // DOG POSES — Quadruped keypoint layout (Pomsky, ~35cm shoulder height)
  // ======================================================================
  //
  // Keypoint mapping for dogs:
  //   0: nose/snout   1: left ear   2: right ear   3: back of head/neck
  //   4: tail tip     5: left front shoulder (withers)   6: right front shoulder
  //   7: left front elbow   8: right front elbow
  //   9: left front paw    10: right front paw
  //  11: left rear hip     12: right rear hip
  //  13: left rear stifle  14: right rear stifle
  //  15: left rear paw     16: right rear paw

  generateDogKeypoints(person, elapsed, breathPulse) {
    const pose = person.pose || 'walking';
    const pos = person.position || [0, 0, 0];
    const facing = person.facing || 0;
    const px = pos[0], pz = pos[2];
    const ms = person.motion_score || 0;
    ...
    let kps;
    switch (pose) {
      case 'lying':    kps = this.dogLying(px, pz, elapsed, bp); break;
      case 'sitting':  kps = this.dogSitting(px, pz, elapsed, bp); break;
      case 'standing': kps = this.dogStanding(px, pz, elapsed, ms, bp); break;
      case 'walking':
      default:         kps = this.dogWalking(px, pz, elapsed, ms, bp); break;
    }
  ...
  // ---- Dog Walking (trot gait — diagonal pairs move together) ----------

  dogWalking(px, pz, elapsed, ms, bp) {
    const speed = Math.min(ms / 80, 3.0);
    const wp = elapsed * speed * 2.5;

    // Trot: diagonal pairs (left-front + right-rear, right-front + left-rear)
    const strideF = Math.sin(wp) * 0.12;
    const strideB = Math.sin(wp + Math.PI) * 0.12;
    const liftF = Math.max(0, Math.sin(wp)) * 0.04;
    const liftB = Math.max(0, Math.sin(wp + Math.PI)) * 0.04;

    // Body bob (double frequency)
    const bob = Math.abs(Math.sin(wp)) * 0.012;

    // Tail wag — faster with more motion
    const tailWag = Math.sin(elapsed * 8 + ms * 0.05) * 0.08;
    const tailUp = 0.12 + Math.abs(tailWag) * 0.3;

    // Head bob with slight sniffing
    const headBob = Math.abs(Math.sin(wp - 0.15)) * 0.008;
    const sniff = Math.sin(elapsed * 3.5) * 0.006;

    // Ear bounce
    const earBounce = Math.abs(Math.sin(wp * 2)) * 0.01;

    // Breathing — barrel chest expansion
    const chest = bp * 0.006;

    return [
      [px, 0.32 + bob + headBob + sniff, pz + 0.38],                              // 0 nose
      [px - 0.06, 0.38 + bob - earBounce, pz + 0.28],                             // 1 left ear
      [px + 0.06, 0.38 + bob - earBounce, pz + 0.28],                             // 2 right ear
      [px, 0.36 + bob, pz + 0.25],                                                 // 3 neck/head back
      [px + tailWag, 0.30 + tailUp, pz - 0.45],                                    // 4 tail tip
      [px - 0.08 + chest, 0.35 + bob + bp, pz + 0.12],                            // 5 left front shoulder
      [px + 0.08 + chest, 0.35 + bob + bp, pz + 0.12],                            // 6 right front shoulder
      [px - 0.08, 0.18 + bob, pz + 0.14 + strideF * 0.3],                         // 7 left front elbow
      [px + 0.08, 0.18 + bob, pz + 0.14 + strideB * 0.3],                         // 8 right front elbow
      [px - 0.08, 0.02 + liftF, pz + 0.14 + strideF],                             // 9 left front paw
      [px + 0.08, 0.02 + liftB, pz + 0.14 + strideB],                             // 10 right front paw
      [px - 0.09, 0.34 + bob + bp * 0.5, pz - 0.18],                              // 11 left rear hip
      [px + 0.09, 0.34 + bob + bp * 0.5, pz - 0.18],                              // 12 right rear hip
      [px - 0.09, 0.16 + bob, pz - 0.20 + strideB * 0.3],                         // 13 left rear stifle
      [px + 0.09, 0.16 + bob, pz - 0.20 + strideF * 0.3],                         // 14 right rear stifle
      [px - 0.09, 0.02 + liftB, pz - 0.22 + strideB],                             // 15 left rear paw
      [px + 0.09, 0.02 + liftF, pz - 0.22 + strideF],                             // 16 right rear paw
    ];
  }

  // ---- Dog Standing (idle with sniffing, ear twitches, tail sway) ------

  dogStanding(px, pz, elapsed, ms, bp) {
    const sway = Math.sin(elapsed * 0.5) * 0.004;
    const sniff = Math.sin(elapsed * 2.0) * 0.012;
    const headTurn = Math.sin(elapsed * 0.4) * 0.02;
    const tailSway = Math.sin(elapsed * 2.5) * 0.05;
    const earTwitch = Math.sin(elapsed * 1.8) * 0.008;
    const chest = bp * 0.006;
    const weightShift = Math.sin(elapsed * 0.3) * 0.005;

    return [
      [px + headTurn, 0.33 + sniff * 0.3, pz + 0.38 + sniff],                    // 0 nose
      [px - 0.06 + headTurn * 0.5, 0.39 + earTwitch, pz + 0.28],                 // 1 left ear
      [px + 0.06 + headTurn * 0.5, 0.39 - earTwitch, pz + 0.28],                 // 2 right ear
      [px + headTurn * 0.3, 0.37, pz + 0.25],                                     // 3 neck
      [px + tailSway, 0.28, pz - 0.42],                                            // 4 tail tip
      [px - 0.08 + chest + weightShift, 0.35 + bp, pz + 0.12],                    // 5 left front shoulder
      [px + 0.08 + chest + weightShift, 0.35 + bp, pz + 0.12],                    // 6 right front shoulder
      [px - 0.08 + sway, 0.18, pz + 0.13],                                        // 7 left front elbow
      [px + 0.08 + sway, 0.18, pz + 0.13],                                        // 8 right front elbow
      [px - 0.08 + weightShift, 0.02, pz + 0.13],                                 // 9 left front paw
      [px + 0.08 + weightShift, 0.02, pz + 0.13],                                 // 10 right front paw
      [px - 0.09, 0.34 + bp * 0.5, pz - 0.18],                                   // 11 left rear hip
      [px + 0.09, 0.34 + bp * 0.5, pz - 0.18],                                   // 12 right rear hip
      [px - 0.09, 0.16, pz - 0.20],                                               // 13 left rear stifle
      [px + 0.09, 0.16, pz - 0.20],                                               // 14 right rear stifle
      [px - 0.09, 0.02, pz - 0.22],                                               // 15 left rear paw
      [px + 0.09, 0.02, pz - 0.22],                                               // 16 right rear paw
    ];
  }

  // ---- Dog Sitting (haunches down, front legs straight) ----------------

  dogSitting(px, pz, elapsed, bp) {
    const tailThump = Math.max(0, Math.sin(elapsed * 3)) * 0.03;
    const pant = Math.sin(elapsed * 4) * 0.004;
    const headTilt = Math.sin(elapsed * 0.6) * 0.015;
    const earFlick = Math.sin(elapsed * 2.2) * 0.006;
    const chest = bp * 0.005;

    return [
      [px + headTilt, 0.38 + pant, pz + 0.30 + pant],                            // 0 nose
      [px - 0.06 + headTilt * 0.3, 0.42 + earFlick, pz + 0.22],                  // 1 left ear
      [px + 0.06 + headTilt * 0.3, 0.42 - earFlick, pz + 0.22],                  // 2 right ear
      [px, 0.40, pz + 0.18],                                                       // 3 neck
      [px + tailThump, 0.06, pz - 0.32],                                           // 4 tail tip (on ground)
      [px - 0.08 + chest, 0.35 + bp, pz + 0.10],                                 // 5 left front shoulder
      [px + 0.08 + chest, 0.35 + bp, pz + 0.10],                                 // 6 right front shoulder
      [px - 0.08, 0.18, pz + 0.12],                                               // 7 left front elbow
      [px + 0.08, 0.18, pz + 0.12],                                               // 8 right front elbow
      [px - 0.08, 0.02, pz + 0.14],                                               // 9 left front paw
      [px + 0.08, 0.02, pz + 0.14],                                               // 10 right front paw
      [px - 0.10, 0.12, pz - 0.10],                                               // 11 left rear hip (lowered)
      [px + 0.10, 0.12, pz - 0.10],                                               // 12 right rear hip
      [px - 0.12, 0.08, pz - 0.04],                                               // 13 left rear stifle (folded)
      [px + 0.12, 0.08, pz - 0.04],                                               // 14 right rear stifle
      [px - 0.10, 0.02, pz - 0.16],                                               // 15 left rear paw (tucked)
      [px + 0.10, 0.02, pz - 0.16],                                               // 16 right rear paw
    ];
  }

  // ---- Dog Lying (curled/flat, breathing visible) ----------------------

  dogLying(px, pz, elapsed, bp) {
    const breathSide = Math.sin(elapsed * 0.9) * 0.005;
    const tailThump = Math.max(0, Math.sin(elapsed * 1.5)) * 0.025;
    const earTwitch = Math.sin(elapsed * 0.7) * 0.004;
    const pawTwitch = Math.sin(elapsed * 0.5) * 0.003;

    return [
      [px, 0.08, pz + 0.30],                                                      // 0 nose
      [px - 0.05, 0.10 + earTwitch, pz + 0.22],                                  // 1 left ear
      [px + 0.05, 0.10 - earTwitch, pz + 0.22],                                  // 2 right ear
      [px, 0.10, pz + 0.18],                                                       // 3 neck
      [px + tailThump, 0.04, pz - 0.35],                                           // 4 tail tip
      [px - 0.10 + breathSide, 0.12 + bp * 0.5, pz + 0.08],                      // 5 left front shoulder
      [px + 0.10 + breathSide, 0.12 + bp * 0.5, pz + 0.08],                      // 6 right front shoulder
      [px - 0.14, 0.05, pz + 0.14],                                               // 7 left front elbow
      [px + 0.06, 0.05, pz + 0.14],                                               // 8 right front elbow
      [px - 0.16 + pawTwitch, 0.02, pz + 0.22],                                  // 9 left front paw
      [px + 0.04 + pawTwitch, 0.02, pz + 0.22],                                  // 10 right front paw
      [px - 0.08 + breathSide, 0.10 + bp * 0.3, pz - 0.12],                      // 11 left rear hip
      [px + 0.08 + breathSide, 0.10 + bp * 0.3, pz - 0.12],                      // 12 right rear hip
      [px - 0.14, 0.06, pz - 0.06],                                               // 13 left rear stifle
      [px + 0.14, 0.06, pz - 0.06],                                               // 14 right rear stifle
      [px - 0.12, 0.02, pz - 0.20],                                               // 15 left rear paw
      [px + 0.12, 0.02, pz - 0.20],                                               // 16 right rear paw
    ];
  }
}
